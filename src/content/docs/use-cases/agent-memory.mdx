...
<CardGrid>
  ...
  <Card title="Robotics agents" icon="star">
    Keep sensor fusion data, action outcomes, operator interventions, and policy notes together as replayable episodes. The Physical AI Action-Outcome line is currently research-only; see the grounded overview before treating it as a production feature.
  </Card>
...
For robot and embodied-agent memory, read [Physical AI Memory and Action-Outcome Memory](/blog/physical-ai-action-outcome-memory/). It separates available ROS 2/time-series/Agent Memory product surfaces from research-only Action-Outcome experiments.
