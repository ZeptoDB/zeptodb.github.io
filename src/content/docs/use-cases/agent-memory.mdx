...
For robot and embodied-agent memory, read [why the robot-memory experiment changed the roadmap](/blog/physical-ai-action-outcome-memory/), then see [Action-Outcome Memory](/use-cases/action-outcome-memory/) and the [experiment evidence](/research/action-outcome-evidence/) for the current controlled-pilot boundary.
