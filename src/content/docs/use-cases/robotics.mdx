...
ZeptoDB's current product surface covers ROS 2 ingest, typed profile tables, rosbag2 import/replay, time-series SQL, ASOF JOIN, Window JOIN, zero-copy Python, and Agent Memory. The more specific Physical AI Action-Outcome Memory experiments are research-only; they are useful evidence for the product direction, not a claim that ZeptoDB is a production robot controller.
