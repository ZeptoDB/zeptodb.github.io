...
Operational databases often need to integrate with existing identity infrastructure. ZeptoDB's SSO journey went from static JWT keys to a full multi-IdP authentication system with OIDC discovery, server-side sessions, and token refresh — all without breaking the existing API key auth.
...
Real organizations often use multiple identity providers. `SsoIdentityProvider` routes tokens by issuer:
